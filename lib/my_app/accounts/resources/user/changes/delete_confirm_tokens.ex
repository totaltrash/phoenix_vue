defmodule MyApp.Accounts.User.Changes.DeleteConfirmTokens do
  ...
  def change(changeset, _opts, _context) do
    if Ash.Changeset.get_argument(changeset, :delete_confirm_tokens) do
      Ash.Changeset.after_action(changeset, fn _changeset, user ->
        days = MyApp.Accounts.User.Helpers.days_for_token("confirm")

        {:ok, query} =
          MyApp.Accounts.UserToken
          |> Ash.Query.filter(
            created_date > ago(^days, :day) and context == "confirm" and
              sent_to == user.email
          )
